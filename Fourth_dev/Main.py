...
def acquire_signal(data_io1, data_io2, data_io3, data_io4, n_samples):
    ...
    try:
        ...
        while True:
            ...
            for _io in [io_1, io_2, io_3, io_4]:
                if _io.typ == "X":
                    ...
                    if clear == "True" and _io.response == 1:
                        plc_client.write_coil((2048 + int(_io.bit)), False)
                else:
                    io_1.response = -2
        ...
        logger(
            [datetime.datetime.now().strftime("%H:%M:%S.%f"), "inf", 0, 0, 0, 0, err]
        )
